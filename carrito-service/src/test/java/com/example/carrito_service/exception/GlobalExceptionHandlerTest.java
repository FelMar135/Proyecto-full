...
class GlobalExceptionHandlerTest {
    ...
    void handleResourceNotFoundException_deberiaRetornarNotFound() {
        ...
        request.setRequestURI("/carritos/99");

        ResponseEntity<ApiErrorResponse> response =
                handler.handleResourceNotFoundException(
                        new ResourceNotFoundException(
                                "Registro de carrito no encontrado con ID: 99"),
                        request
                );
        ...
        assertEquals("Recurso no encontrado",
                response.getBody().getError());
        assertEquals("/carritos/99",
    ...
    void handleBadRequestException_deberiaRetornarBadRequest() {
        ...
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setRequestURI("/carritos");
