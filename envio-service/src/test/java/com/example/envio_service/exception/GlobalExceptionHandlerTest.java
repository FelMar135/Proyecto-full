...
class GlobalExceptionHandlerTest {
    ...
    void handleResourceNotFoundException_deberiaRetornarNotFound() {
        ...
        request.setRequestURI("/envios/99");

        ResponseEntity<ApiErrorResponse> response = handler.handleResourceNotFoundException(
                new ResourceNotFoundException("No existe un envío con ID: 99"),
                request
        );
        ...
        assertEquals("/envios/99", response.getBody().getPath());
    }

    @Test
    void handleBadRequestException_deberiaRetornarBadRequest() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setRequestURI("/envios");
        ...
        assertEquals("/envios", response.getBody().getPath());
    }
